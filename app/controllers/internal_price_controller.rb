...
class InternalPriceController < ApplicationController
  ...
  def all_health_systems
    ...
    render json: {data: data, total_count: health_system_names.count}, status: :ok
  end
  ...
  def reimbursement
    ...
    contract_pharmacy_details = @contract_pharmacy.map do |details|
    {
    contract_pharmacy_group: details, claim_count: reimbursement_spread_claim_count(params[:hospital_name]&.gsub('_', ' '), details, params[:sort]),
